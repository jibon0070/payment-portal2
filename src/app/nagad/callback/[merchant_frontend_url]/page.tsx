...
export default async function Callback(props: Props) {
  ...
  try {
    ...
    const nagadResponse = await getNagadResponse(searchParams.payment_ref_id);
...
async function processBackendData(
  ...
  const merchant_response = await fetch(
    `${process.env.BACKEND_PROCESS_URL}/payment/process`,
    {
      method: "POST",
